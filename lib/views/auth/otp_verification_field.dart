...
import 'package:jhatpat/services/providers/phone_number_provider.dart';
import 'package:jhatpat/services/providers/token_provider.dart';
...
class OTPVerificationFieldState extends State<OTPVerificationField> {
  ...
  Widget build(BuildContext context) {
    debugPrint("🪟 Otp Verification Screen");
    return Form(
      key: _otpGlobalKey,
      child: Column(
        children: <Widget>[
        ...
        try {
          ...
          if (verifiedOrNot) {
            await UserSharedPreferences.setLoggedInOrNot(true)
              ...
              try {
                final bookingDetails = await DatabaseService(
                ...
                debugPrint("❗ OTP Verify Button : $e");
                Navigator.pushAndRemoveUntil(
                    context,
                    CupertinoPageRoute(
            ...
            await UserSharedPreferences.setLoggedInOrNot(false).whenComplete(
                () => showSnackBar(
                    context: context, content: "OTP does not match"));
            _otp = "";
            _otpController.clear();
          }
        } catch (e) {
          debugPrint("VERIFY OTP BUTTON > $e");
          showSnackBar(
              context: context,
              content: "Something went wrong, please try again");
        }
      } else {
        showSnackBar(context: context, content: "No phone number provided");
        setState(() => loading = false);
      }
    }
  }
